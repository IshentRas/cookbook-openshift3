...
path_bin = node['is_apaas_openshift_cookbook']['openshift_docker_etcd_image'].include?('coreos') ? '/usr/local/bin/etcd' : '/usr/bin/etcd'

unless new_etcd_servers.empty?
  if is_certificate_server

    directory node['is_apaas_openshift_cookbook']['etcd_generated_scaleup_dir'] do
      mode '0755'
      owner 'apache'
      group 'apache'
      recursive true
    ...
    new_etcd_servers.each do |etcd|
      execute "Add #{etcd['fqdn']} to the cluster" do
        command "/usr/bin/etcdctl --cert-file #{node['is_apaas_openshift_cookbook']['etcd_generated_certs_dir']}/etcd-#{first_etcd['fqdn']}/peer.crt --key-file #{node['is_apaas_openshift_cookbook']['etcd_generated_certs_dir']}/etcd-#{first_etcd['fqdn']}/peer.key --ca-file #{node['is_apaas_openshift_cookbook']['etcd_generated_ca_dir']}/ca.crt -C #{etcds} member add #{etcd['fqdn']} https://#{etcd['ipaddress']}:2380 | grep ^ETCD | tr --delete '\"' | tee #{node['is_apaas_openshift_cookbook']['etcd_generated_scaleup_dir']}/etcd-#{etcd['fqdn']} && chmod 644 #{node['is_apaas_openshift_cookbook']['etcd_generated_scaleup_dir']}/etcd-#{etcd['fqdn']}"
  ...
  if is_new_etcd_server
    ...
    unless ::File.file?("#{node['is_apaas_openshift_cookbook']['etcd_data_dir']}/.joined")
      ...
      remote_file "Retrieve ETCD SystemD Drop-in from Certificate Server[#{certificate_server['fqdn']}]" do
        ...
        source "http://#{certificate_server['ipaddress']}:#{node['is_apaas_openshift_cookbook']['httpd_xfer_port']}/etcd/scaleup/etcd-#{node['fqdn']}"
      ...
      execute 'Check cluster health' do
        command "[[ $(/usr/bin/etcdctl --cert-file #{node['is_apaas_openshift_cookbook']['etcd_peer_file']} --key-file #{node['is_apaas_openshift_cookbook']['etcd_peer_key']} --ca-file #{node['is_apaas_openshift_cookbook']['etcd_ca_cert']} -C https://`hostname -i`:2379 cluster-health | grep -c 'got healthy') -eq #{new_etcd_size} ]]"
        retries 60
        retry_delay 5
      end
      ...
      file "#{node['is_apaas_openshift_cookbook']['etcd_data_dir']}/.joined" do
        action :create_if_missing
      end
    end
  end
