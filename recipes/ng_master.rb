...
if is_master_server
  ...
  directory "#{node['is_apaas_openshift_cookbook']['openshift_common_base_dir']}/service-catalog" do
    recursive true
    owner 'root'
    group 'root'
    mode '0755'
  ...
  execute 'Create the policy file' do
    command "#{node['is_apaas_openshift_cookbook']['openshift_client_binary']} adm create-bootstrap-policy-file --filename=#{node['is_apaas_openshift_cookbook']['openshift_master_policy']}"
  ...
  template node['is_apaas_openshift_cookbook']['openshift_master_identity_provider']['HTPasswdPasswordIdentityProvider']['filename'] do
    source 'htpasswd.erb'
    mode '600'
    only_if { node['is_apaas_openshift_cookbook']['oauth_Identities'].include? 'HTPasswdPasswordIdentityProvider' }
  end
  ...
  template '/etc/origin/node/pods/apiserver.yaml' do
    ...
    variables(
      api_image: "#{node['is_apaas_openshift_cookbook']['openshift_docker_control-plane_image']}:#{docker_version}"
    )
    mode '0600'
    owner 'root'
    group 'root'
  ...
  template '/etc/origin/node/pods/controller.yaml' do
    ...
    variables(
      controller_image: "#{node['is_apaas_openshift_cookbook']['openshift_docker_control-plane_image']}:#{docker_version}"
    )
    mode '0600'
    owner 'root'
    group 'root'
  ...
  %w[bootstrap.kubeconfig node.kubeconfig].each do |bootstrap|
    remote_file "/etc/origin/node/#{bootstrap}" do
      ...
      notifies :start, 'service[atomic-openshift-node]', :immediately if bootstrap == 'node.kubeconfig'
    end
  end
  ...
  %w[api controllers etcd].each do |item|
    next if item == 'etcd' && !node['is_apaas_openshift_cookbook']['openshift_etcd_static_pod']

    execute "Wait for control plane #{item} pod to appear" do
      command "#{node['is_apaas_openshift_cookbook']['openshift_client_binary']} get pod ${pod} -n kube-system -o jsonpath=\"{range .status.conditions[*]}{@.type}={@.status};{end}\" | grep -w \"Ready=True\""
      environment(
  ...
  node['is_apaas_openshift_cookbook']['openshift_core_api_list'].each do |api|
    execute "Wait for or APIs (#{api}) to become available" do
      command "#{node['is_apaas_openshift_cookbook']['openshift_client_binary']} get --raw /apis/#{api}/v1"
      retries 60
      retry_delay 2
    end
  end
...
include_recipe 'is_apaas_openshift_cookbook::ng_nodes_certificates' if is_certificate_server

if is_first_master
  service_accounts.each do |serviceaccount|
    execute "Creation of namespace \"#{serviceaccount['namespace']}\"" do
      command "#{node['is_apaas_openshift_cookbook']['openshift_client_binary']} adm new-project ${namespace} ${selector} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig"
      environment(
        'namespace' => serviceaccount['namespace'],
        'selector' => serviceaccount.key?('selector') ? "--node-selector=#{serviceaccount['selector']}" : ''
      )
      not_if "#{node['is_apaas_openshift_cookbook']['openshift_client_binary']} get project #{serviceaccount['namespace']} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig"
    end

    execute "Creation service account: \"#{serviceaccount['name']}\" ; Namespace: \"#{serviceaccount['namespace']}\"" do
      command "#{node['is_apaas_openshift_cookbook']['openshift_client_binary']} create sa ${serviceaccount} -n ${namespace} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig"
      environment(
        'serviceaccount' => serviceaccount['name'],
        'namespace' => serviceaccount['namespace']
      )
      not_if "#{node['is_apaas_openshift_cookbook']['openshift_client_binary']} get sa #{serviceaccount['name']} -n #{serviceaccount['namespace']} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig"
    end
    ...
    if node['is_apaas_openshift_cookbook']['openshift_hosted_router_deploy_shards']
      node['is_apaas_openshift_cookbook']['openshift_hosted_router_shard'].each do |shard|
        execute "Add cluster-reader for router sharing service account: \"#{shard['service_account']}\"" do
          command "#{node['is_apaas_openshift_cookbook']['openshift_client_binary']} adm policy add-cluster-role-to-user cluster-reader ${serviceaccount} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig"
          environment(
            'serviceaccount' => "system:serviceaccount:#{shard['namespace']}:#{shard['service_account']}"
          )
          not_if "#{node['is_apaas_openshift_cookbook']['openshift_client_binary']} get clusterrolebinding/cluster-readers -o yaml --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig | grep ${serviceaccount}"
        end
      end
    end
    ...
    sccs.each do |scc|
      execute "Add SCC [#{scc}] to service account: \"#{serviceaccount['name']}\" ; Namespace: \"#{serviceaccount['namespace']}\"" do
        command "#{node['is_apaas_openshift_cookbook']['openshift_client_binary']} adm policy add-scc-to-user #{scc} -z #{serviceaccount['name']} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig -n #{serviceaccount['namespace']}"
        not_if "#{node['is_apaas_openshift_cookbook']['openshift_client_binary']} get scc/#{scc} -n #{serviceaccount['namespace']} -o yaml --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig | grep system:serviceaccount:#{serviceaccount['namespace']}:#{serviceaccount['name']}"
      end
    end
  end
  ...
  node['is_apaas_openshift_cookbook']['image_streams'].each do |imagestream|
    execute "Import Openshift Examples Base image-streams #{imagestream.split('.')[0]}" do
      command "#{node['is_apaas_openshift_cookbook']['openshift_client_binary']} apply -f #{node['is_apaas_openshift_cookbook']['openshift_common_examples_base']}/image-streams/#{imagestream} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig -n openshift"
  ...
  execute 'Import Openshift db templates' do
    command "#{node['is_apaas_openshift_cookbook']['openshift_client_binary']} apply -f #{node['is_apaas_openshift_cookbook']['openshift_common_examples_base']}/db-templates --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig -n openshift"
  ...
  execute 'Import Openshift Examples quickstart-templates' do
    command "#{node['is_apaas_openshift_cookbook']['openshift_client_binary']} apply -f #{node['is_apaas_openshift_cookbook']['openshift_common_examples_base']}/quickstart-templates --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig -n openshift"
  ...
  execute 'Import Openshift Examples xpaas-streams' do
    command "#{node['is_apaas_openshift_cookbook']['openshift_client_binary']} apply -f #{node['is_apaas_openshift_cookbook']['openshift_common_examples_base']}/xpaas-streams --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig -n openshift"
  ...
  execute 'Import Openshift Examples xpaas-templates' do
    command "#{node['is_apaas_openshift_cookbook']['openshift_client_binary']} apply -f #{node['is_apaas_openshift_cookbook']['openshift_common_examples_base']}/xpaas-templates --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig -n openshift"
  ...
  openshift_create_node_config 'Deploy Service Catalog' do
    action :create_kube_service_catalog
  end

  openshift_create_node_config 'Deploy Template Service Broker' do
    action :create_template_service_broker
    only_if { helper.check_pod_not_ready?('openshift-template-service-broker', 'apiserver=true', 1) }
  end

  include_recipe 'is_apaas_openshift_cookbook::web_console'
  ...
  openshift_deploy_logging 'Remove Cluster Logging' do
    ...
    only_if do
      node['is_apaas_openshift_cookbook']['openshift_hosted_cluster_logging'] &&
        !node['is_apaas_openshift_cookbook']['openshift_logging_install_logging']
    end
  end
