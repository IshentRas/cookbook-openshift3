...
service_accounts.each do |serviceaccount|
  execute "Creation of namespace \"#{serviceaccount['namespace']}\"" do
    command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} adm new-project ${namespace} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig --server #{node['is_apaas_openshift_cookbook']['openshift_master_loopback_api_url']}"
    environment(
      'namespace' => serviceaccount['namespace']
    )
    cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
    not_if "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} get project #{serviceaccount['namespace']} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig --server #{node['is_apaas_openshift_cookbook']['openshift_master_loopback_api_url']}"
  end

  execute "Creation service account: \"#{serviceaccount['name']}\" ; Namespace: \"#{serviceaccount['namespace']}\"" do
    command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} create sa ${serviceaccount} -n ${namespace} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig --server #{node['is_apaas_openshift_cookbook']['openshift_master_loopback_api_url']}"
    environment(
      'serviceaccount' => serviceaccount['name'],
      'namespace' => serviceaccount['namespace']
    )
    cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
    not_if "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} get sa #{serviceaccount['name']} -n #{serviceaccount['namespace']} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig --server #{node['is_apaas_openshift_cookbook']['openshift_master_loopback_api_url']}"
  end
  ...
  if node['is_apaas_openshift_cookbook']['openshift_hosted_router_deploy_shards']
    node['is_apaas_openshift_cookbook']['openshift_hosted_router_shard'].each do |shard|
      execute "Add cluster-reader for router sharing service account: \"#{shard['service_account']}\"" do
        command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} adm policy add-cluster-role-to-user cluster-reader ${serviceaccount} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig --server #{node['is_apaas_openshift_cookbook']['openshift_master_loopback_api_url']}"
        environment(
          'serviceaccount' => "system:serviceaccount:#{shard['namespace']}:#{shard['service_account']}"
        )
        cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
        not_if "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} get clusterrolebinding/cluster-readers -o yaml --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig --server #{node['is_apaas_openshift_cookbook']['openshift_master_loopback_api_url']} | grep ${serviceaccount}"
      end
    end
  end
  ...
  sccs.each do |scc|
    execute "Add SCC [#{scc}] to service account: \"#{serviceaccount['name']}\" ; Namespace: \"#{serviceaccount['namespace']}\"" do
      command "#{node['is_apaas_openshift_cookbook']['openshift_common_admin_binary']} policy add-scc-to-user #{scc} -z #{serviceaccount['name']} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig -n #{serviceaccount['namespace']} --server #{node['is_apaas_openshift_cookbook']['openshift_master_loopback_api_url']}"
      cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
      not_if "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} get scc/#{scc} -n #{serviceaccount['namespace']} -o yaml --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig --server #{node['is_apaas_openshift_cookbook']['openshift_master_loopback_api_url']} | grep system:serviceaccount:#{serviceaccount['namespace']}:#{serviceaccount['name']}"
    end
  end
end
...
execute 'Import Openshift db templates' do
  command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} apply -f #{node['is_apaas_openshift_cookbook']['openshift_common_examples_base']}/db-templates --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig -n openshift --server #{node['is_apaas_openshift_cookbook']['openshift_master_loopback_api_url']}"
  cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
...
execute 'Import Openshift Examples Base image-streams' do
  command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} apply -f #{node['is_apaas_openshift_cookbook']['openshift_common_examples_base']}/image-streams/#{node['is_apaas_openshift_cookbook']['openshift_base_images']} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig -n openshift --server #{node['is_apaas_openshift_cookbook']['openshift_master_loopback_api_url']}"
  cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
...
execute 'Import Openshift Examples quickstart-templates' do
  command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} apply -f #{node['is_apaas_openshift_cookbook']['openshift_common_examples_base']}/quickstart-templates --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig -n openshift --server #{node['is_apaas_openshift_cookbook']['openshift_master_loopback_api_url']}"
  cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
...
execute 'Import Openshift Examples xpaas-streams' do
  command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} apply -f #{node['is_apaas_openshift_cookbook']['openshift_common_examples_base']}/xpaas-streams --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig -n openshift --server #{node['is_apaas_openshift_cookbook']['openshift_master_loopback_api_url']}"
  cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
...
execute 'Import Openshift Examples xpaas-templates' do
  command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} apply -f #{node['is_apaas_openshift_cookbook']['openshift_common_examples_base']}/xpaas-templates --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig -n openshift --server #{node['is_apaas_openshift_cookbook']['openshift_master_loopback_api_url']}"
  cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
...
node_servers.reject { |h| h.key?('skip_run') }.each do |nodes|
  ...
  execute "Set Labels for node : #{nodes['fqdn']}" do
    command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} label node #{nodes['fqdn']} ${labels} --overwrite --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig --server #{node['is_apaas_openshift_cookbook']['openshift_master_loopback_api_url']}"
    environment(
...
if ose_major_version.split('.')[1].to_i >= 9
  master_servers.each do |master_server|
    execute "Set \"master\" label for master : #{master_server['fqdn']}" do
      command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} label node #{master_server['fqdn']} ${labels} --overwrite --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig --server #{node['is_apaas_openshift_cookbook']['openshift_master_loopback_api_url']}"
      environment(
        'labels' => 'node-role.kubernetes.io/master=true'
      )
      cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
      not_if do
        Mixlib::ShellOut.new("#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} get node --server #{node['is_apaas_openshift_cookbook']['openshift_master_loopback_api_url']} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig | grep #{master_server['fqdn']}").run_command.error?
      end
    end
  end
  ...
  node_servers.each do |node_server|
    execute "Set \"compute\" label for node : #{node_server['fqdn']}" do
      ...
      cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
      not_if do
        Mixlib::ShellOut.new("#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} get node --server #{node['is_apaas_openshift_cookbook']['openshift_master_loopback_api_url']} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig | grep #{node_server['fqdn']}").run_command.error?
      end
    end
  end
