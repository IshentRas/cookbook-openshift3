...
action :create do
  converge_by "Deploy Router on #{node['fqdn']}" do
    ...
    execute 'Deploy Hosted Router' do
      command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} adm router #{deploy_options.join(' ')} --images=#{node['is_apaas_openshift_cookbook']['openshift_docker_hosted_router_image']} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig || true"
      environment(
        'selector_router' => node['is_apaas_openshift_cookbook']['openshift_hosted_router_selector'],
        'namespace_router' => node['is_apaas_openshift_cookbook']['openshift_hosted_router_namespace']
      )
      cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
      only_if "[[ `#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} get pod --selector=router=router -n ${namespace_router} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig | wc -l` -eq 0 ]]"
    end

    if node['is_apaas_openshift_cookbook']['openshift_hosted_router_deploy_shards']
      node['is_apaas_openshift_cookbook']['openshift_hosted_router_shard'].each do |shard|
        execute "Deploy Hosted Router for sharding[#{shard['service_account']}]" do
          command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} adm router router-#{shard['service_account']} --images=#{node['is_apaas_openshift_cookbook']['openshift_docker_hosted_router_image']} --selector=${selector_router} --service-account=#{shard['service_account']} -n ${namespace_router} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig || true"
          environment(
            'selector_router' => shard['selector'],
            'namespace_router' => shard['namespace']
          )
          cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
          only_if "[[ `#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} get pod --selector=router=router-#{shard['service_account']} -n ${namespace_router} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig | wc -l` -eq 0 ]]"
        end
      end
    end
    ...
    if node['is_apaas_openshift_cookbook']['openshift_hosted_router_deploy_shards']
      node['is_apaas_openshift_cookbook']['openshift_hosted_router_shard'].each do |shard|
        execute "Auto Scale Router based on label for sharding[#{shard['service_account']}]" do
          command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} scale dc/router-#{shard['service_account']} --replicas=${replica_number} -n ${namespace_router} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig"
          environment(
            'replica_number' => Mixlib::ShellOut.new("#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} get node --no-headers --selector=#{shard['selector']} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig | wc -l").run_command.stdout.strip,
            'selector_router' => shard['selector'],
            'namespace_router' => shard['namespace']
          )
          cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
          not_if "[[ `#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} get pod --selector=router=router-#{shard['service_account']} -n ${namespace_router} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig --no-headers | wc -l` -eq ${replica_number} ]]"
        end
      end
    end
      ...
      node['is_apaas_openshift_cookbook']['openshift_hosted_router_shard'].each do |shard|
        shard['env'].each do |env|
          execute "Set Sharding ENV #{env} for Hosted Router sharding[#{shard['service_account']}]" do
            command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} set env dc/router-#{shard['service_account']} #{env} -n ${namespace_router} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig"
            environment(
              'selector_router' => shard['selector'],
              'namespace_router' => shard['namespace']
            )
            cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
            not_if "[[ `#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} env dc/router-#{shard['service_account']} --list -n ${namespace_router} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig` =~ \"#{env}\" ]]"
          end
        end
      end
    end
      ...
      if node['is_apaas_openshift_cookbook']['openshift_hosted_router_deploy_shards']
        node['is_apaas_openshift_cookbook']['openshift_hosted_router_shard'].each do |shard|
          execute "Create ConfigMap of the customised Hosted Router sharding[#{shard['service_account']}]" do
            command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} create configmap customrouter --from-file=haproxy-config.template=${custom_router_file} -n ${namespace_router} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig"
            environment(
              'namespace_router' => shard['namespace'],
          ...
          execute "Set ENV TEMPLATE_FILE for customised Hosted Router sharding[#{shard['service_account']}]" do
            command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} set env dc/router-#{shard['service_account']} TEMPLATE_FILE=/var/lib/haproxy/conf/custom/haproxy-config.template -n ${namespace_router} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig"
            environment(
              'namespace_router' => shard['namespace']
            )
            cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
            not_if "[[ `#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} env dc/router-#{shard['service_account']} --list -n ${namespace_router} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig` =~ \"TEMPLATE_FILE=/var/lib/haproxy/conf/custom/haproxy-config.template\" ]]"
          end

          execute "Set Volume for customised Hosted Router sharding[#{shard['service_account']}]" do
            command "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} volume dc/router-#{shard['service_account']} --add --name=#{node['is_apaas_openshift_cookbook']['openshift_hosted_deploy_custom_name']} --mount-path=/var/lib/haproxy/conf/custom --type=configmap --configmap-name=customrouter -n ${namespace_router} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig"
            environment(
              'namespace_router' => shard['namespace']
            )
            cwd node['is_apaas_openshift_cookbook']['openshift_master_config_dir']
            not_if "#{node['is_apaas_openshift_cookbook']['openshift_common_client_binary']} volume dc/router-#{shard['service_account']} -n ${namespace_router} --config=#{node['is_apaas_openshift_cookbook']['openshift_master_config_dir']}/admin.kubeconfig | grep /var/lib/haproxy/conf/custom"
          end
        end
      end
    end
